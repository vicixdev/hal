...
case "$target_api" in
	Vulkan)
		...
		odin test -collection:shared=shared -debug -out:build/tests -define:ODIN_TEST_THREADS=4 -define:GFX_TARGET_API=Vulkan src/gfx/tests
	;;
	Metal_3)
		...
		odin test -collection:shared=shared -debug -out:build/tests -define:ODIN_TEST_THREADS=4 -define:GFX_TARGET_API=Metal_3 src/gfx/tests
	;;
	*)
