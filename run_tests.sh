...
case "$target_api" in
	Vulkan)
		...
		slangc ./src/gfx/tests/triangle_with_transform.slang -profile glsl_450 -target spirv -o ./src/gfx/tests/shaders/triangle_with_transform.spv -fvk-use-entrypoint-name -DGFX_VULKAN=1
		odin test -debug -out:build/tests -define:ODIN_TEST_THREADS=4 -define:GFX_TARGET_API=Vulkan -keep-executable src/gfx/tests
	;;
	Metal_3)
		...
		slangc ./src/gfx/tests/triangle_with_transform.slang -target metallib -o ./src/gfx/tests/shaders/triangle_with_transform.metallib -fvk-use-entrypoint-name -DGFX_METAL=1
		odin test -debug -out:build/tests -define:ODIN_TEST_THREADS=4 -define:GFX_TARGET_API=Metal_3 -keep-executable src/gfx/tests
	;;
	*)
