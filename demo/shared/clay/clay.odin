...
ID :: proc(label: string, #any_int index: u32 = 0) -> ElementId {
	return _HashString(MakeString(label), index)
}

ID_LOCAL :: proc(label: string, #any_int index: u32 = 0) -> ElementId {
	return _HashStringWithOffset(MakeString(label), index, GetOpenElementId())
}

StringToOdin :: proc(str: String) -> string {
	return strings.string_from_ptr(str.chars, cast(int)str.length)
}

StringSliceToOdin :: proc(str: StringSlice) -> string {
	return strings.string_from_ptr(str.chars, cast(int)str.length)
}
