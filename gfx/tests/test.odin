...
import imgwrite "vendor:stb/image"
import gfx ".."
...
test_against_reference_image :: proc(
	...
	splits := strings.split(REFERENCE_PATH, "/", context.temp_allocator)
	write_path := fmt.ctprintf("./.test_outputs/%s", splits[len(splits) - 1])
	imgwrite.write_png(
		write_path,
		cast(i32)reference.width,
		cast(i32)reference.height,
		cast(i32)reference.channels,
		raw_data(pixels),
		cast(i32)(reference.width * reference.channels),
	)
...
init_gfx :: proc "contextless" () {
	...
	if !os.exists("./.test_outputs") {
		os.make_directory("./.test_outputs")
	}
